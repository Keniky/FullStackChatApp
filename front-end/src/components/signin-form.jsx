...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float glow = smoothstep(0.7, 0.2, wave);
...
export function SignInForm() {
  ...
  const signin = async (e) => {
    //fetch sign in first then log in 
    // /api/v2/signin
    e.preventDefault();
    ...
    const res = await fetch("http://localhost:80/api/v2/signin",{
        method: "POST",
        body: formData
    })
    if(!res.ok){
    ...
    //i managed to successfulyl upload the data lol and now i have an account
    ...
    <div
      ...
      <div className="text-center">
        ...
        <form method="POST" onSubmit={signin}>
          <label className="rounded-full border-gray-500 mb-2 border-solid border-[2px] p-2"> 
        ...
        {/* <button
          ...
          <svg className="w-5 h-5 mr-2" viewBox="0 0 48 48">
            <path
              fill="#FFC107"
              d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039L38.802 8.841C34.553 4.806 29.613 2.5 24 2.5C11.983 2.5 2.5 11.983 2.5 24s9.483 21.5 21.5 21.5S45.5 36.017 45.5 24c0-1.538-.135-3.022-.389-4.417z"></path><path
            fill="#FF3D00"
            d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12.5 24 12.5c3.059 0 5.842 1.154 7.961 3.039l5.839-5.841C34.553 4.806 29.613 2.5 24 2.5C16.318 2.5 9.642 6.723 6.306 14.691z"></path><path
            fill="#4CAF50"
            d="M24 45.5c5.613 0 10.553-2.306 14.802-6.341l-5.839-5.841C30.842 35.846 27.059 38 24 38c-5.039 0-9.345-2.608-11.124-6.481l-6.571 4.819C9.642 41.277 16.318 45.5 24 45.5z"></path><path
            fill="#1976D2"
            d="M43.611 20.083H42V20H24v8h11.303c-.792 2.237-2.231 4.166-4.087 5.571l5.839 5.841C44.196 35.123 45.5 29.837 45.5 24c0-1.538-.135-3.022-.389-4.417z"></path>
          </svg>
          Sign in with Google
        </button> */}
      ...
      {/* <p className="text-center text-xs text-gray-400">
       Don't have an account? <a
        href="#"
        className="font-semibold text-blue-400 hover:text-blue-300 transition">Sign Up</a>
     </p> */}
    </div>
  );
}